...
class AdminSettings {
	...
	public function __construct() {
		...
		$theme_info = ( $theme_info->parent() ) ?: $theme_info;
		$listzen  = $theme_info->get( 'Name' );

		// Theme name.
		$this->listzen = $listzen;

		// Theme slug.
		$listzen        = strtolower( trim( preg_replace( '/[^A-Za-z0-9-]+/', '-', $listzen ) ) );
		$this->theme_slug = $listzen;
	}
	...
	public function theme_menu() {
		add_theme_page( esc_html__( 'Theme License', 'listzen' ), esc_html__( 'Theme License', 'listzen' ), 'manage_options', 'rtlc', [ $this, 'create_admin_page' ], null, 99 );
	}
	...
	public function create_admin_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Theme License', 'listzen' ); ?></h1>
			<form method="post" action="options.php">
				<?php
				// This prints out all hidden setting fields.
				settings_fields( 'rt_option_group' );
				do_settings_sections( 'listzen-setting' );
				?>
			</form>
		</div>
		<?php
	...
	public function theme_option() {
		...
		add_settings_field(
			'rt_purchase_code',
			esc_html__( 'Purchase Code', 'listzen' ),
		...
		add_settings_field(
			'rt_license_status',
			esc_html__( 'License Status', 'listzen' ),
	...
	public function license_status_callback() {
		...
		$status_text = esc_html__( 'Not Activated', 'listzen' );

		if ( rtlc_is_valid()['success'] ) {
			$verify      = true;
			$status_text = __( 'Activated', 'listzen' );
		} elseif ( isset( rtlc_is_valid()['domain_match'] ) && ! rtlc_is_valid()['domain_match'] ) {
			$status_text = __( 'Domain Mismatch', 'listzen' );
		}
	...
	public function license_note_callback() {
		...
		/* translators: Support Center */
			__( 'Please keep in mind, you can activate one license in one domain, if you face any problem in activation, please contact our <a href="%s" target="_blank">Support Center</a>', 'listzen' ),
			esc_url( $support )
		);
	...
	public function rtlc_verification() {
        check_ajax_referer( 'rtlc_verification_nonce' );
        ...
        if ( empty( $purchase_code ) ) {
            wp_send_json_error( 'Empty purchase code' );
        }
		...
		if ( $purchase_code ) {
			...
			$listzen   = $this->listzen;
			$domain_name = listzen_get_domain_name();

			$api_url     = "{$rt_license_server}/?theme_name={$listzen}&purchase_code=" . $purchase_code . '&domain_name=' . $domain_name;
			...
			if ( $envato_data ) {
				...
				echo json_decode( $envato_data ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
