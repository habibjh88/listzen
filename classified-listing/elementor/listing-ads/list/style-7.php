			...
			while ( $the_loops->have_posts() ) :
                ...
                <div <?php Functions::listing_class( [ 'rtcl-widget-listing-item', 'listing-item', 'rtcl-listing-card', 'style-category-thumb', $img_position_class ] ); ?>>
					...
					if ( $instance['rtcl_show_details_button'] ) {
						$button_lable        = ! empty($instance['rtcl_details_btn_label']) ? $instance['rtcl_details_btn_label'] : __( 'Details', 'listzen' );
						$item_details_button = sprintf(
							'<a class="rtin-details-button" href="%s">%s</a>',
							get_the_permalink(),
							esc_html( $button_lable )
						);
					}
					...
					$item_content   = sprintf(
						'<div class="item-content">%s %s %s %s %s %s %s %s %s %s</div>%s',
						$author_image,
						$listing_rating,
						$labels,
						$category,
